...
#     return True
def check_collective_obscuration(missile_pos, active_cloud_centers, target_key_points):
    """
    判断在给定时刻，多个烟幕云团是否协同地完全遮蔽了目标。
    使用覆盖整个目标外表面的关键点集进行判断。
    
    Args:
        missile_pos (np.array): 导弹位置。
        active_cloud_centers (list of np.array): 所有有效烟幕云团中心的位置列表。
        target_key_points (np.array): 代表目标外表面的N x 3关键点数组。
    
    Returns:
        bool: 如果协同遮蔽成功，返回 True。
    """
    ...
    # 1. 为每一个云团构建其对应的阴影锥
    cones = []
    for cloud_center in active_cloud_centers:
        cone_vertex = missile_pos
        vec_vc = cloud_center - cone_vertex
        dist = np.linalg.norm(vec_vc)
        ...
        axis = vec_vc / dist
        angle = np.arcsin(CLOUD_RADIUS / dist)
        cones.append({'axis': axis, 'half_angle': angle})
    ...
    for point in target_key_points:
        ...
        for cone in cones:
            ...
            cos_beta = np.dot(vec_vp, cone['axis']) / vec_vp_norm
            beta = np.arccos(np.clip(cos_beta, -1.0, 1.0))
