...
class TargetCylinder:
    def __init__(self, specs, num_circ_samples=16, num_height_samples=5):
        ...
        # 在初始化时就生成并缓存关键点
        self._key_points = self._generate_full_key_points(num_circ_samples, num_height_samples)

    def get_key_points(self):
        """返回预先计算好的关键点集。"""
        return self._key_points

    def _generate_full_key_points(self, num_circ_samples, num_height_samples):
        """
        生成能代表整个圆柱体外表面的关键点集，包括上下底面和侧面。
        """
        points = []
        
        # 1. 上下底面圆盘的采样 (圆周 + 圆心)
        points.append(self.bottom_center)
        points.append(self.top_center)
        for i in range(num_circ_samples):
            angle = 2 * np.pi * i / num_circ_samples
            # 假设圆柱体是竖直的，底面平行于xy平面
            offset_xy = np.array([self.radius * np.cos(angle), self.radius * np.sin(angle), 0.0])
            
            # 底面圆周点
            points.append(self.bottom_center + offset_xy)
            # 顶面圆周点 (修正: top_center 已经是顶部圆心)
            points.append(self.top_center + offset_xy)
            
        # 2. 侧面母线的采样
        # 在4个方向（0, 90, 180, 270度）的母线上进行采样
        num_side_samples = 4
        for i in range(num_side_samples):
            ...
            for j in range(1, num_height_samples):
                ...
                height_offset = np.array([0.0, 0.0, self.height * height_fraction])
                points.append(self.bottom_center + offset_xy + height_offset)
